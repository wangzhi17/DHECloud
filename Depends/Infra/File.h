...
public:
	...
	/// \retval >=0  写入的字节数
	/// \retval <0 写失败
	virtual long writev(const struct io_vec *iov, int iovcnt);
...
/// \brief 大文件服务封装
////////////////////////////////////////////////////////////////////////////////
struct LfsFileInternal;

class INFRA_API CLfsFile
{
	CLfsFile(CFile const&);
	CLfsFile& operator=(CFile const&);
...
/// \brief 使用 Infra::CString 的文件查找类，不可作为基类继承，支持 WIN32 DLL
...
public:
	...
	virtual size_t writev(int fileds, const struct io_vec *iov, int iovcnt) { return 0; };
	virtual int fseek64(FileHandle, int64_t, int) {return -1;};
	virtual int64_t ftell64(FileHandle) {return -1;};
};
