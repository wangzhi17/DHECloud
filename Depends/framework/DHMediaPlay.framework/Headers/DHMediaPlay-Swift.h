...
#ifndef DHMEDIAPLAY_SWIFT_H
#define DHMEDIAPLAY_SWIFT_H
...
/// 用于记录音轨选择当码流播放成功后需要再次设置。0是音轨1，1是音轨2
@property (nonatomic) NSInteger audioChannelId;
...
/// 是否是用户或上层强制暂停,这样换页的时候就不会再复播,指的是用户暂停了该窗口
...
typedef SWIFT_ENUM(NSInteger, ControlType, open) {
  ControlTypeNotdeal = 0,
  ControlTypeVolume = 1,
  ControlTypeBrightness = 2,
  ControlTypeScorll = 3,
};
...
typedef SWIFT_ENUM(NSInteger, DHMediaLogLevel, open) {
...
- (BOOL)needAnimationViewWithWinIndex:(NSUInteger)winIndex SWIFT_WARN_UNUSED_RESULT;
- (NSArray<UIImage *> * _Nullable)animationImagesWithWinIndex:(NSUInteger)winIndex SWIFT_WARN_UNUSED_RESULT;
@end

enum PageCountViewLocation : NSInteger;
...
/// 设置pageControl 隐藏和显示
@property (nonatomic) enum PageCountViewLocation pageCountViewLocation;
/// 设置是否会回传码流量
@property (nonatomic) BOOL shouldSendBitrate;
/// 设置是否自动调整分辨率，适应屏幕宽高
@property (nonatomic) BOOL autoAdjustResolution;
/// 添加对讲监听
/// \param listener 监听者
///
...
/// 当camera信息变更时，对应的播放组件参数也需要同步
/// \param winIndex 窗口序列号
///
- (void)changePlayParams:(NSUInteger)winIndex;
/// 全部播放，有camera的窗口都会播放
///
/// returns:
...
/// 开启电子放大的能力（全部窗口），当窗口最大化的时候就可电子放大了
/// \param enable 开启或关闭
///
- (void)setEnableElectricZoomGlobal:(BOOL)enable;
/// 重置电子放大倍数
/// \param winIndex 窗口序列号
///
...
/// \param enable 开启或关闭
///
- (void)setEnableTemperature:(NSUInteger)winIndex enable:(BOOL)enable;
/// 全局开启冻结拖动
/// \param enable 开启或关闭
///
- (void)setEnableFreezeMode:(BOOL)enable;
/// 开启冻结拖动
/// \param winIndex 窗口序列号
///
/// \param enable 开启或关闭
...
- (NSInteger)getAudioChannelNum:(NSUInteger)winIndex SWIFT_WARN_UNUSED_RESULT;
- (void)chooseAudioChannel:(NSUInteger)winIndex channelId:(NSInteger)channelId;
/// 是否正在音频播放
/// \param winIndex 窗口序列号
///
///
...
@property (nonatomic) BOOL shouldDoubleClick;
...
/// returns:
/// 抓图的路径,当返回nil时表示不成功
- (NSString * _Nullable)startRecord:(NSUInteger)winIndex filePath:(NSString * _Nullable)filePath withAudio:(BOOL)withAudio SWIFT_WARN_UNUSED_RESULT;
/// 开始录像，会抓一张图，再生成.dav或.mp4的录像文件（名称相同，后缀修改）
/// \param winIndex 窗口序列号
///
/// \param filePath 抓图的路径，可不传默认返回一个,这个路径需要传jpg结尾的文件，作为抓图的路径
...
typedef SWIFT_ENUM(NSInteger, DHPlaySDKLogLevel, open) {
...
/// 声光控制事件的回调
/// \param controlType 控制的类型
///
/// \param value 控制值
///
- (void)volumeBrightnessControled:(enum ControlType)controlType value:(float)value;
/// 声光控制结束事件的回调
- (void)volumeBrightnessEnded;
/// 测温点位点击事件的回调
/// \param winIndex 窗口号
///
/// \param point cell中的点位位置
///
/// \param pointIn8192 传化为8192坐标系后的点位
///
- (void)onTemperatureClicked:(NSUInteger)winIndex point:(CGPoint)point pointIn8192:(CGPoint)pointIn8192;
@end
...
/// 是否启用播放组件range传值，【可选】默认 1代表启用, 0代表不启用
@property (nonatomic) NSInteger rangeEnable;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
typedef SWIFT_ENUM(NSInteger, FISHEYE_CALIBRATMODE, open) {
...
typedef SWIFT_ENUM(NSInteger, FISHEYE_MOUNTMODE, open) {
...
typedef SWIFT_ENUM(NSInteger, FISHEYE_winType, open) {
...
/// 是否OpenAPI平台
@property (nonatomic) BOOL isOpenSDKPlatForm;
/// 开放平台需要的token
@property (nonatomic, copy) NSString * _Nonnull openAPIToken;
/// 开放平台需要的token
@property (nonatomic, copy) NSString * _Nonnull openPlayCode;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
///   </li>
/// </ul>
typedef SWIFT_ENUM(NSInteger, PageChangeType, open) {
  PageChangeTypeMaximize = 0,
  PageChangeTypeMinimize = 1,
  PageChangeTypeScroll = 2,
};

typedef SWIFT_ENUM(NSInteger, PageCountViewLocation, open) {
  PageCountViewLocationTop = 0,
  PageCountViewLocationBottom = 1,
};

typedef SWIFT_ENUM(NSInteger, PageCountViewStyle, open) {
...
///   </li>
/// </ul>
typedef SWIFT_ENUM(NSInteger, PlayStatusType, open) {
...
///   </li>
/// </ul>
typedef SWIFT_ENUM(NSInteger, PtzOperation, open) {
  PtzOperationBegin = 0,
...
typedef SWIFT_ENUM(NSInteger, SlidingDirection, open) {
  SlidingDirectionLeftOrRight = 0,
  SlidingDirectionUpOrDown = 1,
  SlidingDirectionNone = 2,
};
...
///   </li>
/// </ul>
typedef SWIFT_ENUM(NSInteger, TalkResultType, open) {
...
///   </li>
/// </ul>
typedef SWIFT_ENUM(NSInteger, WinControlType, open) {
  WinControlTypeAdd = 0,
